...
struct TreeNode {
    string key; // non stop words appearing in the corpus text
    List *context; // context associated with the key
...
class BinarySearchTree {
    ...
    // Function to display tree in readable form
    friend void display(TreeNode*);  
    TreeNode* getNode(string); // Create a new node by allocating memory
public:
    BinarySearchTree(); // constructor
    ~BinarySearchTree(); // destructor
    TreeNode* getRootNode(void)const; // returns root node of tree
    // Creates the node with information provided in arguments
    // inserts the node at appropriate position in tree
    void insert(string, string*&, string*&); 
    void buildTree(ifstream&, string*&,int);
...
// If the word has punctuation or colon, remove the characters and insert
...
void BinarySearchTree::buildTree(ifstream& inputfile, 
    ...
    // Keep reading the words until EOF is encountered
    do {   
        ...
        insert(tempCurr, beforeTheWord, afterTheWord);
